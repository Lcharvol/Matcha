...
const Notif = {
  ...
  getAllConversation(userSend, userReceive) {
    ...
    return this.db.any(`SELECT * FROM chat WHERE (user_send = ${Number(userSend)} AND user_receive = ${Number(userReceive)}) OR (user_send = ${Number(userReceive)} AND user_receive = ${Number(userSend)}) ORDER BY date`);
  },
};
